...
| --- | --- | --- |
| A local application uses the proxy to spend the Kiro credential | loopback bind, mandatory local token, constant-time compare, 0600 token file | spec 6.3, security tests `health_needs_no_token_but_everything_else_does`, `only_loopback_addresses_are_accepted` |
| Credential exfiltration through a redirected or substituted upstream | `Destination` enum is the only host input, strict region pattern and runtime allowlist, HTTPS only, redirects rejected, proxy variables ignored, compiled-in TLS roots | spec 6.2, security tests `oidc_redirect_rejected`, `region_pattern`, `runtime_allowlist`, `proxy_env_ignored`; "`Destination` is the only host input" itself is unpinned (spec 8.4) |
| A diagnostic probe sends credentials or leaves loopback | `doctor --network` fixes HTTP/1 `GET /health` to a validated loopback socket, with no token, caller headers, proxy, or redirects | spec 4.6, 6.2; `loopback_health_probe_accepts_only_the_fixed_unauthenticated_health_response`, `loopback_health_probe_rejects_non_loopback_addresses`, `loopback_health_probe_rejects_redirects` |
